...
export default function HomePage() {
            ...
            getStocksData().then(res => {
                ...
                res.forEach((stock, index) => {
                    if (stock.events_data) {
                        ...
                        domElement.style.cssText = `background-color:${FLICKER_COLOR};transition:1.5s all;`;
                        setTimeout(() => {
                            domElement.style.cssText = `background-color:${NAVBAR_COLOR};transition:1.5s all;`;
                            setTimeout(() => {
                                domElement.style.cssText = `background-color:${FLICKER_COLOR};transition:1.5s all;`;
                                setTimeout(() => {
                                    domElement.style.cssText = `background-color:${NAVBAR_COLOR};transition:1.5s all;`;
    ...
    const fullColumns = [
        ...
        },
        {
            ...
            dataIndex: 'price_change',
            render: priceChange => <RowStyled
                color={priceChange < 0 ? PRICE_DOWN : PRICE_UP}>{priceChange < 0 ? '' : '+'}{priceChange % 1 !== 0 ? priceChange.toFixed(2) : priceChange}</RowStyled>,
            align: 'center',
        ...
        },
        {
            ...
            align: 'center',
            render: volume =>
                <RowStyled>{volume % 1 !== 0 ? volume.toFixed(2).toLocaleString() : volume.toLocaleString()}</RowStyled>,
        ...
        },
        {
            ...
            render: interest => <RowStyled
                color={interest < 0 ? PRICE_DOWN : PRICE_UP}>{interest < 0 ? '' : '+'}{interest % 1 !== 0 ? interest.toFixed(2) : interest}%</RowStyled>,
            align: 'center',
            sorter: (a, b) => a.current_change - b.current_change
        },
        {
            title: <TitleStyle traffic>Last Updated</TitleStyle>,
            dataIndex: 'last_event',
            align: 'center',
            render: last_event => <RowStyled color={PRIMARY}>{last_event}</RowStyled>,
            sorter: (a, b) => a.last_event - b.last_event
            ...
            <Row>
                ...
                <Col span={3}>
                    <SearchStyled style={{marginLeft: 70}}>
                        <Input prefix={<SearchOutlined style={{fontSize: 16}}/>} placeholder={'add ticker'} style={{
                            borderRadius: 8,
                ...
                <Col span={2}>
                    <ButtonStyled>
                        <Button style={{backgroundColor: `${BACKGROUND_COLOR}`, color: 'white', borderRadius: 6}}>add
                            portfolio</Button>
                    </ButtonStyled>
                </Col>
            </Row>
